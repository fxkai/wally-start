...
{  // expects value of form ffffff
  ...
  if (hexValue.length() == 6)
    {
      result = strtol(hexValue.c_str(), NULL, 16);
...
void PixelList::crossfade(PixelList otherParent, int steps)
{
  for (int i = 0; i < steps; i++)
      ...
      for (int j = 0; j < NUM_LEDS; j++)
	{
	  ...
	  myRed += sign * (fabs(otherRed-myRed) / (steps - i));
	  sign = myGreen > otherGreen ? -1 : 1;
	  myGreen += sign * fabs(otherGreen-myGreen) / (steps - i);
	  sign = myBlue > otherBlue ? -1 : 1;
	  myBlue += sign * fabs(otherBlue-myBlue) / (steps - i);
